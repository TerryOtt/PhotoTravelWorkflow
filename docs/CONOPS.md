...
| No GPX tracks on the laptop — forgot the copy, or the logger died | Pre-flight refuses. Copy the tracks in (or point `--gpx` at them). If none exist tonight, `offload --no-gpx` lands the raws untagged; when tracks turn up, re-run before the next format. **If the cards are already formatted, `offload geotag` is the fix** — see below. |
| **Tracks turned up after the cards were wiped** | **`offload geotag <archive-root> <track.gpx>`** tags the landed raws in place, no cards involved (decision 30). This case had no remedy at all until 2026-08-07: the re-run above needs the cards, and a formatted card ends that option. **Run it with `--dry-run` first** — it reports what would be tagged and writes nothing. |
