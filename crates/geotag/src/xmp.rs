...
impl fmt::Display for Packet<'_> {
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        writeln!(
            f,
            r#"<x:xmpmeta xmlns:x="adobe:ns:meta/" x:xmptk="{}">"#,
            self.writer
        )?;
        writeln!(
            f,
...
///
/// `writer` becomes the packet's `x:xmptk`, the XMP toolkit field — the tool and
/// version that wrote this sidecar. It is a parameter rather than this crate's own
/// name because two tools now share this engine, and a sidecar naming the wrong one
/// is a small lie in a file whose entire job is provenance. Callers pass
...
/// than the allocation.
pub fn render(fix: &Fix, captured: DateTime<Utc>, writer: &str) -> String {
    Packet {
        fix,
        captured,
        writer,
...
/// filesystem. `tempfile` gives it a random name — two processes over
...
mod tests {
    ...
    /// A writer identity that is nobody's real one, so a packet asserted byte for
    /// byte cannot accidentally match a hard-coded name.
    const WRITER: &str = "testkit 1.2.3";
    ...
    fn the_packet_is_exactly_this() {
        ...
        let xmpmeta = r#"<x:xmpmeta xmlns:x="adobe:ns:meta/" x:xmptk="testkit 1.2.3">"#.to_string();
        let expected = [
            "<?xpacket begin=\"\u{feff}\" id=\"W5M0MpCehiHzreSzNTczkc9d\"?>",
            &xmpmeta,
            r#" <rdf:RDF xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#">"#,
        ...
        assert_eq!(render(&fix, captured(), WRITER), expected);
    }

    /// The toolkit field is the caller's, verbatim. Pinned because it is the one
    /// value in the packet that no longer comes from this crate: a `render` that
    /// ignored the argument and hard-coded a name again would still pass every other
    /// test here, since they all use the same writer.
    #[test]
    fn the_toolkit_field_is_the_writer_the_caller_named() {
        let fix = Fix {
            lat: 0.0,
            lon: 0.0,
            ele: None,
    ...
    fn packet_omits_altitude_entirely_when_the_track_has_none() {
        ...
        let packet = render(&fix, captured(), WRITER);
