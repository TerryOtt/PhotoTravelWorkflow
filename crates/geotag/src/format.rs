...
//! differently from a CR3 one. RawGeotag's CLAUDE.md has the measurements.
//!
...
impl RawFormat {
    ...
    /// see the NEF section of RawGeotag's CLAUDE.md for what the wrong choice looks like.
...
mod tests {
    ...
    /// reads the per-file manifests actually committed under `fixture-manifests/`
    /// and checks that some real file carries each format's extension. Add a
    /// `RawFormat` without adding a fixture and this fails, naming what is missing.
    ///
    ...
    ///
    /// **The manifests came across with the lift; the fixture *files* did not.**
    /// They are raw frames of tens of megabytes each and belong in neither
    /// repository — RawGeotag holds the corpus and `scripts/verify-fixtures.ps1`,
    /// which is what actually hashes them. What this test guards is the weaker but
    /// still load-bearing half: that the format table never claims support the
    /// verification set has never seen a file of.
    #[test]
    fn every_format_has_a_fixture_of_its_own() {
        let manifests = Path::new(env!("CARGO_MANIFEST_DIR")).join("fixture-manifests");
        ...
        for format in RawFormat::ALL {
            assert!(
                ...
                "{format:?} has no fixture: nothing under fixture-manifests/ carries \
                 any of {:?}. A format is not supported until a real file of it is in the \
                 verification set — see RawGeotag's docs/FIXTURES.md.",
                format.extensions()
            );
        }
    }
