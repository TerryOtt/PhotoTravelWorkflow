...
/// Re-exported because both entry points below take one, so a caller cannot use this
/// module without naming the type. Without this every consumer would need its own
/// direct `nom-exif` dependency purely to spell an argument — and would then be free
/// to drift onto a different version of it than the one this crate parses with.
...
pub enum Capture {
    ...
    Resolved {
        ...
        /// saying out loud — see *Whose clock is it* in RawGeotag's CLAUDE.md.
        offset: FixedOffset,
...
///
/// **`read_strategy` still applies, and honoring it is not redundant here.** It is
/// tempting to reason that bytes already in memory make the distinction moot; they do
/// not. The strategy describes which *parser path* a format survives, not how much of
/// the file to read — NEF fails through the seeking path and parses only through the
/// memory one (see `format.rs`). So a `Streaming` format gets a cursor over the
/// caller's slice and copies nothing, and a `WholeFile` format pays one copy because
/// that is the only path on which it parses at all.
pub fn capture_time_in_memory(
    parser: &mut MediaParser,
    bytes: &[u8],
    format: RawFormat,
    utc_offset: Option<FixedOffset>,
    ...
    let parsed =
        match format.read_strategy() {
            ReadStrategy::Streaming => MediaSource::seekable(Cursor::new(bytes))
                .and_then(|source| parser.parse_exif(source)),
            ReadStrategy::WholeFile => MediaSource::from_memory(bytes.to_vec())
                .and_then(|source| parser.parse_exif(source)),
        };
...
///
/// Split out when the in-memory entry point was added. The two public functions differ
/// only in how they build a `MediaSource`; keeping the timezone policy — which is the
/// part that can silently misplace a photo by a day of travel — in one body is what
/// stops the two paths from ever drifting into disagreeing about it.
fn resolve(
    parsed: std::result::Result<ExifIter, ExifError>,
    format: RawFormat,
    utc_offset: Option<FixedOffset>,
) -> std::result::Result<Capture, ExifError> {
...
/// pre-merge. That is the CR3 timezone trap; see RawGeotag's CLAUDE.md before touching it.
///
